...
moduleForComponent('uxs-form-time', 'Integration | Component | uxs form time', {
  integration: true
});

test('it renders a default time input', function(assert) {
  ...
  this.render(hbs `{{uxs-form-time date=date name="date" onChange=(action (mut date))}}`);
  ...
  assert.equal($input.attr("class"), 'uxs-form__input uxs-form__input--time ember-view flatpickr-input', 'Has correct classes');
...
test('it renders a model bound time input', function(assert) {
  ...
  this.render(hbs `{{uxs-form-time model=model name="date" onChange=(action (mut model.date))}}`);
